...
YmaKmern 当前定位为通用智能 QQ 聊天 Agent，不再提供校园、课程、考试、教务或学业数据能力。QQ 消息明确涉及学校事务时，入口会直接静默，不发送提示、进度或替代答复。
...
- 学校相关消息在最前置入口静默，不调用模型、工具或用户可见进度。
- 保留时间、天气、新闻、翻译、联网搜索与完整聊天 Agent 能力。
...
- 模型只有提交通过校验的外部工具步骤时，才能把普通消息提升为工具请求；天气、搜索等明确查询不受影响
...
- 普通闲聊不再枚举、过滤或规划任何工具；只有规则/感知明确标记需要外部能力时才进入通用工具链
...
- 慢任务进度提示阈值默认 5 秒，且只在已进入工具查询阶段时发送；普通闲聊即使模型较慢也不显示“正在分析需求”
- `DUDUDA_TURN_MERGE_DELAY`（默认 `0.45` 秒）控制相邻短气泡的静默合并窗口，`DUDUDA_TURN_MERGE_MAX_DELAY`（默认 `1.2` 秒）限制最长等待；完整长句、带句末标点的消息和媒体不固定等待
